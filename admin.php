...
SELECT concat(a.Id, CURRENT_DATE()+1), now(), a.UserId, a.ProductId, a.Quantity, (a.Quantity * c.Amount)
FROM Subscription AS a, User AS b , Product AS c
WHERE a.UserId = b.Id AND a.ProductId = c.Id AND
(a.PausedFrom IS NULL OR a.PausedFrom > NOW() OR a.PausedTo < NOW()) AND
 ((a.Type = 'D') OR
(a.Type = 'W' AND FIND_IN_SET(ELT(DAYOFWEEK(CURDATE() + 1),'Sun', 'Mon', 'Tue', 'Wed', 'Thu', 'Fri', 'Sat') , a.Details)) OR
